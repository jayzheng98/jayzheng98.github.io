...
## Experiment
**1.** We separately implemented the pre and post penetration by Kali in the shooting range. All the behavior data before and after the implementation (**total 3 days**) have been saved as the **raw dataset**
 - *We've collected about 200k sysmon logs in total, and roughly estimate that there are about 10k abnormal data generated by the designed attacks*

**2.** By running the `main.py` in **[detection rules](https://github.com/jayzheng98/Mapping-Sysmonlogs-to-ATTACK)**, it may overwrite the original "RuleName" field of some logs with more precise technique ids and add a new field "RiskLevel" to all logs. Then it will export the processed dataset in `.csv` format (To facilitate the subsequent import into the graph database) 
